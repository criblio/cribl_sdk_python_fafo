"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""

from __future__ import annotations
from .iometricsconfig import IoMetricsConfig, IoMetricsConfigTypedDict
from cribl.types import BaseModel
from cribl.utils import FieldMetadata, PathParamMetadata
from typing import List, Optional
from typing_extensions import Annotated, NotRequired, TypedDict


class GetIoMetricsConfigByIDRequestTypedDict(TypedDict):
    id: str
    r"""Unique ID to GET"""


class GetIoMetricsConfigByIDRequest(BaseModel):
    id: Annotated[
        str, FieldMetadata(path=PathParamMetadata(style="simple", explode=False))
    ]
    r"""Unique ID to GET"""


class GetIoMetricsConfigByIDResponseBodyTypedDict(TypedDict):
    r"""a list of IoMetricsConfig objects"""

    count: NotRequired[int]
    r"""number of items present in the items array"""
    items: NotRequired[List[IoMetricsConfigTypedDict]]


class GetIoMetricsConfigByIDResponseBody(BaseModel):
    r"""a list of IoMetricsConfig objects"""

    count: Optional[int] = None
    r"""number of items present in the items array"""

    items: Optional[List[IoMetricsConfig]] = None
