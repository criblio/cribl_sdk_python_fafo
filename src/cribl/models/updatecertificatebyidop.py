"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""

from __future__ import annotations
from .certificate import Certificate, CertificateTypedDict
from cribl.types import BaseModel
from cribl.utils import FieldMetadata, PathParamMetadata, RequestMetadata
import pydantic
from typing import List, Optional
from typing_extensions import Annotated, NotRequired, TypedDict


class UpdateCertificateByIDRequestTypedDict(TypedDict):
    id_param: str
    r"""Unique ID to PATCH"""
    certificate: CertificateTypedDict
    r"""Certificate object to be updated"""


class UpdateCertificateByIDRequest(BaseModel):
    id_param: Annotated[
        str,
        pydantic.Field(alias="id"),
        FieldMetadata(path=PathParamMetadata(style="simple", explode=False)),
    ]
    r"""Unique ID to PATCH"""

    certificate: Annotated[
        Certificate,
        FieldMetadata(request=RequestMetadata(media_type="application/json")),
    ]
    r"""Certificate object to be updated"""


class UpdateCertificateByIDResponseBodyTypedDict(TypedDict):
    r"""a list of Certificate objects"""

    count: NotRequired[int]
    r"""number of items present in the items array"""
    items: NotRequired[List[CertificateTypedDict]]


class UpdateCertificateByIDResponseBody(BaseModel):
    r"""a list of Certificate objects"""

    count: Optional[int] = None
    r"""number of items present in the items array"""

    items: Optional[List[Certificate]] = None
