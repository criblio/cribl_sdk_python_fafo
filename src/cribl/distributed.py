...
class Distributed(BaseSDK):
    ...
    def get_config_bundles_by_group_and_version(
        self,
        *,
        group: str,
        version: str,
        ...
        r"""Download config bundle (used by remote nodes)
        ...
        request = models.GetConfigBundlesByGroupAndVersionRequest(
            group=group,
            version=version,
        )
        ...
        req = self._build_request(
            method="GET",
            path="/master/bundles/{group}/{version}",
            base_url=base_url,
            url_variables=url_variables,
            request=request,
            request_body_required=False,
        ...
        http_res = self.do_request(
            hook_ctx=HookContext(
                base_url=base_url or "",
                operation_id="getConfigBundlesByGroupAndVersion",
                oauth2_scopes=[],
                security_source=get_security_from_env(
                    self.sdk_configuration.security, models.SecurityModel
                ),
    ...
    async def get_config_bundles_by_group_and_version_async(
        self,
        *,
        group: str,
        version: str,
    ...
    def get_summary(
        ...
        *,
        ...
        response_data: Any = None
        if utils.match_response(http_res, "200", "application/json"):
            return utils.unmarshal_json(http_res.text, models.GetSummaryResponseBody)
        if utils.match_response(http_res, "500", "application/json"):
            response_data = utils.unmarshal_json(http_res.text, models.ErrorData)
            raise models.Error(data=response_data)
        if utils.match_response(http_res, ["401", "4XX"], "*"):
