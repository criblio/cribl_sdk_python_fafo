"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""

from __future__ import annotations
from .pipeline import Pipeline, PipelineTypedDict
from cribl.types import BaseModel
from cribl.utils import FieldMetadata, PathParamMetadata, RequestMetadata
import pydantic
from typing import List, Optional
from typing_extensions import Annotated, NotRequired, TypedDict


class UpdatePipelineByPackAndIDRequestTypedDict(TypedDict):
    id_param: str
    r"""Unique ID to PATCH for pack"""
    pack: str
    r"""pack ID to PATCH"""
    pipeline: PipelineTypedDict
    r"""Pipeline object to be updated"""


class UpdatePipelineByPackAndIDRequest(BaseModel):
    id_param: Annotated[
        str,
        pydantic.Field(alias="id"),
        FieldMetadata(path=PathParamMetadata(style="simple", explode=False)),
    ]
    r"""Unique ID to PATCH for pack"""

    pack: Annotated[
        str, FieldMetadata(path=PathParamMetadata(style="simple", explode=False))
    ]
    r"""pack ID to PATCH"""

    pipeline: Annotated[
        Pipeline, FieldMetadata(request=RequestMetadata(media_type="application/json"))
    ]
    r"""Pipeline object to be updated"""


class UpdatePipelineByPackAndIDResponseBodyTypedDict(TypedDict):
    r"""a list of Pipeline objects"""

    count: NotRequired[int]
    r"""number of items present in the items array"""
    items: NotRequired[List[PipelineTypedDict]]


class UpdatePipelineByPackAndIDResponseBody(BaseModel):
    r"""a list of Pipeline objects"""

    count: Optional[int] = None
    r"""number of items present in the items array"""

    items: Optional[List[Pipeline]] = None
